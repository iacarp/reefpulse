...
export default function EquipmentScreen({ navigation }) {
  ...
  const handleAddFromDB = async (cat, item) => {
    await addEquipment({ category: cat, name: item.name, brand: item.name.split(' ')[0], model: item.name, specs: { size: item.size, price: item.price }, maintenance_schedule_days: 30, notes: item.notes, is_custom: false });
    Alert.alert(t.added, item.name);
    await load(); setTab('my');
  };

  const handleAddCustom = async () => {
    if (!cf.name || !cf.category) { Alert.alert(t.error, 'Name + Category required'); return; }
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingTop: 56, paddingBottom: 80 }}>
        ...
        {tab === 'my' && (<>
          ...
          : myEq.map(eq => {
            ...
            const over = days !== null && days > (eq.maintenance_schedule_days || 30);
            const due = days !== null && days >= (eq.maintenance_schedule_days || 30) * 0.8;
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ color: sc, fontSize: 11, marginTop: 4, fontWeight: '600' }}>
                      {days === null ? t.never : over ? `⚠️ ${days}d (+${days - (eq.maintenance_schedule_days || 30)} ${t.overdue})` : `✓ ${days}/${eq.maintenance_schedule_days || 30}d`}
                    </Text>
                  </View>
                  <View style={{ gap: 6 }}>
            ...
            {EQUIPMENT_DATABASE.categories.find(c => c.name === selCat)?.items.map((item, i) => (
              <View key={i} style={{ backgroundColor: '#0f172a', borderRadius: 14, padding: 14, borderWidth: 1, borderColor: '#1e293b', marginBottom: 8 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <TouchableOpacity onPress={() => handleAddFromDB(selCat, item)} style={{ backgroundColor: '#10b98120', borderWidth: 1, borderColor: '#10b98140', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8 }}>
        ...
        {tab === 'custom' && (
          <Card>
            ...
            <Text style={{ color: '#64748b', fontSize: 11, marginBottom: 4 }}>{t.model}</Text>
            <TextInput value={cf.model} onChangeText={v => setCf({ ...cf, model: v })} placeholderTextColor="#334155" style={IS} />
            <Text style={{ color: '#64748b', fontSize: 11, marginBottom: 4 }}>{t.maintenanceDays}</Text>
            <TextInput keyboardType="number-pad" value={cf.maintenance_schedule_days} onChangeText={v => setCf({ ...cf, maintenance_schedule_days: v })} placeholderTextColor="#334155" style={IS} />
            ...
            {reminders.length > 0 && (<View style={{ marginTop: 12 }}>
              ...
              {reminders.map(r => (
                <View key={r.id} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#1e293b', borderRadius: 8, padding: 8, marginBottom: 4 }}>
                  ...
                  <TouchableOpacity onPress={async () => { await removeReminder(r.id); await load(); }}>
